...
// v2: alerts now broadcast to the public channel, not a DM — start fresh so the
// channel gets a clean first batch instead of inheriting DM-era cooldowns.
const ALERT_STATE = path.join(config.dataDir, 'alert-state-v2.json');
...
function writeAlertState(state) {
  ensureDirs();
  writeAtomic(ALERT_STATE, JSON.stringify(state, null, 2));
}

module.exports = {
  saveSnapshot, readLatest, readPrior, listSnapshotFiles, writeAtomic,
  readAlertState, writeAlertState, SNAP_DIR,
};
