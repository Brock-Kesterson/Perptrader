...
module.exports = {
  ...
  // Broadcast alert engine (public Telegram channel — the free-tier funnel).
  // Per-user custom thresholds are the paid tier, built after Stripe.
  alerts: {
    minOpenInterestUsd: 15_000_000,  // never alert on illiquid coins
    extremeApr: 0.50,                // |APR| >= 50% on a venue -> "funding extreme"
    clearApr: 0.35,                  // must fall back below this before it can re-fire
    spreadApr: 0.30,                 // cross-venue spread >= 30% APR -> "wide spread"
    spreadClearApr: 0.20,
    flipMinApr: 0.15,               // avg funding crossing zero, both sides >= this magnitude
    cooldownHours: 12,               // min gap before the same alert key can re-fire
    maxPerCycle: 6,                  // cap messages per poll so a market-wide move can't spam
    siteUrl: process.env.PERPRADAR_URL || 'https://perpradar.example',
    // Broadcast alerts go here. Public channel @DailyRepoAlerta ("PerpRadar
    // Alerts") — a public channel id is not secret. Override with env if needed.
    chatId: process.env.TELEGRAM_ALERT_CHAT_ID || '-1004359520753',
  },
